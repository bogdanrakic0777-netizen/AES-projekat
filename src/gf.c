#include "gf.h"

uint8_t xtime(uint8_t a) {
    int b = a << 1;
    if(0x100 & b) {
        return (uint8_t) b ^ REDUCE; //ako je b izasao van dodira van dometa posle shifta, "oduzme se ostatak pri deljenju" sa IRREDUCIBLE
    }
    return (uint8_t) b;
}

uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t rezultat = 0;
    if(a == 1) {
        return b;
    }
    if(b == 1) {
        return a;
    }

    while(b != 0) {
        if(b & 0x01) {
            rezultat ^= a;
        }
        a = xtime(a);
        b >>= 1;
    }
    return rezultat;
}

//FIPS 197 standatd
//lookup tabela za inverzni element
static const uint8_t inv_table[256] = {
     
};

//inverz po lookup tabeli
uint8_t gf_inv_table(uint8_t a) {
    return inv_table[a];
}

//stepenovanje
uint8_t gf_pow(uint8_t a, int exp) {
    uint8_t rezultat = 1;
    while (exp) {
        if(0x01 & exp) {
            rezultat = gf_mul(rezultat, a);
        }
        a = gf_mul(a, a);
        exp <<= 1;
    }
    return rezultat;
}

//inverz preko male Fermatove teoreme
uint8_t gf_inv_fermat(uint8_t a) {
    return a ? gf_pow(a, 254) : 0;
}